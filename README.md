...
| Platform | Entities |
|---|---|
| Lawn mower | The robot while a mower head is attached: mowing, paused, returning, docked or error. Start, pause and dock. Start resumes a paused plan; from rest it starts the plan chosen in the Plan selector. |
| Select | Plan: the robot's plans by name. Choosing one sends nothing; it is what Start will start. |
